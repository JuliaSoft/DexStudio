...
/**
 * The table cell displaying the signature of a method
 * 
 * 
...
public class DexMethodCell extends JTextPane {
	...
	public DexMethodCell(MethodGen meth) {
		...
		String color = "rgb(" + getForeground().getRed() + ", "
				+ getForeground().getGreen() + ", " + getForeground().getBlue()
				+ ")";
		String fontName = "Courier";
		int fontSize = getFont().getSize();
		String text = "<html><head><style type=\"text/css\">body{font-family:"
				+ fontName + "; font-weight:normal; font-size:" + fontSize
				+ "pt;color:" + color + ";}</style></head><body>";
		if (meth.isConstructor())
			text += ((meth != null) ? "<a href='meth'>" : "")
					+ Library.printType(meth.getOwnerClass())
					+ ((meth != null) ? "</a>" : "") + "(";
		else {
			text += ((returnType != null) ? "<a href='returnclass'>" : "")
					+ Library.printType(meth.getReturnType())
					+ ((returnType != null) ? "</a>" : "") + " ";
			text += ((meth != null) ? "<a href='meth'>" : "") + meth.getName()
					+ ((meth != null) ? "</a>" : "") + "(";
		}
		int j = 0;
		...
		for (ParamGen param : meth.getParams()) {
			...
			String paramName = ((param.getParamName() != null) ? " "
					+ param.getParamName() : " arg" + j);
			text += ((params[j] != null) ? "<a href='par" + j + "'>" : "")
					+ Library.printType(param.getType())
					+ ((params[j] != null) ? "</a>" : "") + paramName;
			j++;
		}
		text += ")</body></html>";
