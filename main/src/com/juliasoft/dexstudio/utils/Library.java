...
public final class Library {
	...
	public static String printType(Type type){
			...
			default : result += typeName.substring(typeName.lastIndexOf('/')+1, typeName.indexOf(';'));
 	...
 	public static String printSignature(MethodGen meth){
 		...
 		if(!meth.isConstructor())
 			...
 			result += Library.printType(meth.getPrototype().getReturnType()) + " " + meth.getName() + "(";
	...
	public static String printClassFlags(int classFlags){
			...
			for (AccessFlag flag : AccessFlag.values()){
				...
				if( !flag.allowedForClass() || flag.equals(AccessFlag.ACC_INTERFACE) || (flag.equals(AccessFlag.ACC_ABSTRACT) && AccessFlag.ACC_INTERFACE.isSet(classFlags)) )
					continue;
