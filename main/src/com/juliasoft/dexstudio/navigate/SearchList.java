...
public class SearchList extends JTable
{
		...
		this.getColumnModel().getColumn(1).setCellRenderer(new DefaultTableCellRenderer()
		{
			...
			public Component getTableCellRendererComponent(JTable table, Object value, boolean selected, boolean focus, int row, int col)
			{
				...
				else if(value instanceof CompareNode)
				{
					...
					SimpleClassDiff diff = (SimpleClassDiff) node.getDiff();
				...
				if(clazz != null)
				{
					String pkg = clazz.toHuman().substring(0, clazz.toHuman().lastIndexOf('.'));
					label.setText("<html><b>" + name.substring(0, src.length()) + "</b>" + name.substring(src.length()) + " : <span style='color: #999999;'>" + pkg + "</span></html>");
				}
				return label;
			}
		});
	...
	{
		return results.get((this.getSelectedRow() == -1) ? 1 : this.getSelectedRow());
	}
