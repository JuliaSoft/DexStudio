...
/**
 * The top component of the Class tab
 * 
 * 
...
public class DexClassHeader extends JTextPane {
	...
	private final String pakStyle = ".pak{font-family:" + fontName
			+ "; font-size:" + pakFontSize + "; color:" + color
			+ "; font-weight:normal;}";
	...
	private final String htmlFormat = "<html><head><style type=\"text/css\">"
			+ pakStyle + descriptionStyle + "</style></head><body>";
	...
	public DexClassHeader(DexDisplay display, ClassGen clazz) {
		...
		this.superClass = display.getDexGen()
				.getClassGen(clazz.getSuperclass());
		...
		String pakName = className.contains("/") ? className.substring(1,
				className.lastIndexOf('/')) : "[default pakage]";
		pakName = pakName.replace('/', '.');
		String pakage = "<div class='pak'>" + pakName + "</div>";
		String flags = "<b>"
				+ Library.printClassFlags(clazz.getFlags()).replaceAll("[<,>]",
						"") + "</b>";
		String name = (AccessFlag.ACC_INTERFACE.isSet(clazz.getFlags()) ? " <b>interface</b> "
				: " <b>class</b> ")
				+ Library.shortName(className) + "<br />";
		String extendz = "<b>extends:</b> "
				+ ((superClass != null) ? "<a href='superclass'>" : "")
				+ Library.shortName(clazz.getSuperclass().getName()
						.replace('/', '.'))
				+ ((superClass != null) ? "</a>" : "") + "<br />";
		...
		if (interfList == null || interfList.isEmpty())
			...
			for (Type interf : interfList) {
				if (separator)
					...
					separator = true;
				implementz += ((interfaces[i] != null) ? "<a href='int" + i
						+ "'>" : "")
						+ Library.shortName(interf.getName())
						+ ((interfaces[i] != null) ? "</a>" : "");
				i++;
			}
			implementz += "<br />";
		...
		if (annList.isEmpty())
			...
			for (i = 0; i < annotations.length; i++) {
				if (separator)
					...
					separator = true;
				annots += ("<a href='ann" + i + "'>" + "@"
						+ Library.printType(annotations[i].getType()) + "</a>");
			}
		}
		String description = "<div class='description'>" + flags + name
				+ extendz + implementz + annots + "</div>";
			...
			public void hyperlinkUpdate(HyperlinkEvent e) {
				...
				if (((mask & InputEvent.BUTTON1_MASK) == mask)
					...
					if (code.equals("superclass"))
						DexClassHeader.this.display
								.changeSelectedTab(new DexTreeTab(
										DexClassHeader.this.display, superClass));
