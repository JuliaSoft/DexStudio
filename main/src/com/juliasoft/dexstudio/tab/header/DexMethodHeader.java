...
/**
 * The top component of the Method tab
 * 
 * 
...
public class DexMethodHeader extends JTextPane {
	...
	private final String classStyle = ".class{font-family:" + fontName
			+ "; font-size:" + clazzFontSize + "; color:" + color
			+ "; font-weight:normal;}";
	...
	private final String htmlFormat = "<html><head><style type=\"text/css\">"
			+ classStyle + descriptionStyle + "</style></head><body>";
	...
	public DexMethodHeader(DexDisplay display, MethodGen meth) {
		...
		String pakName = longName.contains(".") ? longName.substring(1,
				longName.lastIndexOf('.')) : "[default pakage]";
		...
		String clazz = "<div class='description'>" + pakName + "."
				+ ((ownerClass != null) ? "<a href=\"ownerclass\">" : "")
				+ shortName + ((ownerClass != null) ? "</a>" : "") + "</div>";
		String description = "<div class='class'><b>"
				+ AccessFlag.decodeToHuman(meth.getFlags(), false,
						Const.FLAG_USE_METHOD) + "</b> " + printLinkSignature()
				+ "<br />" + annots + "</div>";
	...
	private String printLinkSignature() {
		...
		if (!meth.isConstructor())
			result += ((returnType != null) ? "<a href='returntype'>" : "")
					+ Library.printType(meth.getPrototype().getReturnType())
					+ ((returnType != null) ? "</a>" : "") + " "
					+ meth.getName() + "(";
		else
		...
		if (parameters != null) {
			...
			for (ParamGen param : meth.getParams()) {
				if (separator)
					...
					separator = true;
				String paramName = ((param.getParamName() != null) ? " "
						+ param.getParamName() : " arg" + i);
				result += ((params[i] != null) ? "<a href='par" + i + "'>" : "")
						+ Library.printType(param.getType())
						+ ((params[i] != null) ? "</a>" : "") + paramName;
				i++;
			}
		}
		result += ")";
