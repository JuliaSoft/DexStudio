...
/**
 * The top component of the Annotation tab
 * 
 * 
...
public class DexAnnotationHeader extends JTextPane {
	...
	private final String htmlFormat = "<html><head><style type=\"text/css\">"
			+ descriptionStyle + pakStyle + "</style></head><body>";
	...
	public DexAnnotationHeader(Annotation ann) {
		...
		String pakage = "<div class='pak'>"
				+ annName.substring(1, annName.lastIndexOf('/')).replace('/',
						'.') + "</div>";
		String description = "<div class='description'>"
				+ "<b>Annotation Type</b> " + "@"
				+ Library.printType(ann.getType()) + "</div>";
		...
		this.setText(htmlFormat + pakage + description + visibility
				+ closeFormat);
