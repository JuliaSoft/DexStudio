	...
	private void initLayout() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (obj instanceof ClassGen) {
					display.changeSelectedTab(new DexTreeTab(display,
							(ClassGen) node.getUserObject()));
				} else if (obj instanceof MethodGen) {
					display.changeSelectedTab(new DexTreeTab(display,
							(MethodGen) node.getUserObject()));
				} else if (obj instanceof Annotation) {
					display.changeSelectedTab(new DexTreeTab(display,
							(Annotation) node.getUserObject()));
				} else if (obj instanceof StringSet) {
					display.changeSelectedTab(new DexTreeTab(display,
							(StringSet) node.getUserObject()));
				}
			}
		});
		...
		newTab.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Object obj = node.getUserObject();
				if (obj instanceof ClassGen) {
					display.openNewTab(new DexTreeTab(display, (ClassGen) node
							.getUserObject()));
				} else if (obj instanceof MethodGen) {
					display.openNewTab(new DexTreeTab(display, (MethodGen) node
							.getUserObject()));
				} else if (obj instanceof Annotation) {
					display.openNewTab(new DexTreeTab(display,
							(Annotation) node.getUserObject()));
				} else if (obj instanceof StringSet) {
					display.openNewTab(new DexTreeTab(display, (StringSet) node
							.getUserObject()));
				}
			}
		});
