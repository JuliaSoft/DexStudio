...
/**
 * Tree representation of the class hierarchy of the target apk
 * 
...
public class DexTree extends DexView {
	...
	private void mouseRightPressed(MouseEvent e) {
		...
		if (node instanceof TreeNode
				&& !((TreeNode) node).getType().equals(NodeType.ROOT)) {
			// Open the popup menu
			new TreePopup(frame, tree, node).show(e.getComponent(), e.getX(),
					e.getY());
		}
	}
	...
	private void mouseDoubleClicked(MouseEvent e) {
		...
		} else if (node.getType().equals(NodeType.CLASS)
				|| node.getType().equals(NodeType.INTERFACE)) {
			frame.changeSelectedTab(new DexTreeTab(frame, (ClassGen) node
					.getUserObject()));
		} else if (node.getType().equals(NodeType.METHOD)) {
			frame.changeSelectedTab(new DexTreeTab(frame, (MethodGen) node
					.getUserObject()));
		} else if (node.getType().equals(NodeType.ANNOTATION)) {
			frame.changeSelectedTab(new DexTreeTab(frame, (Annotation) node
					.getUserObject()));
		} else if (node.getType().equals(NodeType.STRINGS)) {
			frame.changeSelectedTab(new DexTreeTab(frame, (StringSet) node
					.getUserObject()));
		}
	}
	...
	public void updateLayout(DexGen dexGen, String rootLabel) {
		...
		((DexFrame) frame).getViewManager().setTabComponentAt(0,
				this.getTabTitle());
	}
