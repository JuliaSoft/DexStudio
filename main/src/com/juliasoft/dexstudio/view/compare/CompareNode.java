...
/**
 * A node in the tree view of a comparison class
 * 
 * 
...
public class CompareNode extends DefaultMutableTreeNode implements
		Comparable<CompareNode> {
	private String label;
	private NodeType type;
	private DiffNode<?> diff;
	private DiffState state;

	public CompareNode(NodeType type, String name)
			throws IllegalArgumentException {
		if (!type.equals(NodeType.ROOT) && !type.equals(NodeType.FOLDER)
	...
	public CompareNode(NodeType type, DiffNode<?> diff)
			throws IllegalArgumentException {
		if ((type.equals(NodeType.CLASS) || type.equals(NodeType.INTERFACE))
				&& !diff.getDiffClass().equals(ClassGen.class)
				|| type.equals(NodeType.FIELD)
				&& !diff.getDiffClass().equals(FieldGen.class)
				|| type.equals(NodeType.METHOD)
				&& !diff.getDiffClass().equals(MethodGen.class)
				|| type.equals(NodeType.ANNOTATION)
				&& !diff.getDiffClass().equals(Annotation.class)
				|| type.equals(NodeType.STRINGS)
				&& !diff.getDiffClass().equals(ContextGen.class)
				|| type.equals(NodeType.ROOT) || type.equals(NodeType.FOLDER)
				|| type.equals(NodeType.PACKAGE))
			throw new IllegalArgumentException();
		this.type = type;
		this.diff = diff;
		...
		// Create the label
		Object obj = (diff.getState().equals(DiffState.RIGHT_ONLY)) ? diff
				.getRight() : diff.getLeft();
		if (obj instanceof ClassGen) {
			String className = ((ClassGen) obj).getType().getName()
					.replace("/", ".");
			...
			String result = (meth.isConstructor() ? Library.printType(meth
					.getOwnerClass()) : meth.getName()) + "(";
			...
			if (!meth.isConstructor()) {
				result += " : "
						+ Library
								.printType(meth.getPrototype().getReturnType());
			}
			this.label = result;
	...
	public DiffState updateState(DiffState child) {
		...
		case LEFT_ONLY:
			if (child.equals(DiffState.RIGHT_ONLY)
					|| child.equals(DiffState.SAME)
					|| child.equals(DiffState.DIFFERENT))
				return DiffState.DIFFERENT;
			return DiffState.LEFT_ONLY;
		case RIGHT_ONLY:
			if (child.equals(DiffState.LEFT_ONLY)
					|| child.equals(DiffState.SAME)
					|| child.equals(DiffState.DIFFERENT))
				return DiffState.DIFFERENT;
	...
	public int compareTo(CompareNode node) {
		int cmp = this.toString().toLowerCase()
				.compareTo(node.toString().toLowerCase());
		if (cmp == 0)
			return 1;
		return cmp;
	}
